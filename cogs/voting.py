...
class Poll(discord.ui.View):
    def __init__(self, currentPoll, timeout):
    ...
    async def on_timeout(self):
        resultsEmbed = await self.currentPoll.clipboardBot.bot.get_command('createResultsEmbed')(self.currentPoll)
        self.clear_items()
        await self.response.edit(embed = resultsEmbed, view = self)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.emoji.name == '⌛':
            ...
            if view.value == None:
                ...
                pollView = Poll(self.currentPoll, self.currentPoll.timeout)
...
#* ------------   Create Poll   ------------
    ...
    #* Forming the Poll
        ...
        try:
            ...
            #Poll object given attributes for creating poll
            currentPoll = PollClass(ctx, self, embed, fullEmojiList, fullOptionList, timeout)
            
            #Poll View class given CurrentPoll Poll object
            pollView = Poll(currentPoll, timeout)

            out = await ctx.send(embed = embed, view = pollView)
            pollView.response = out
            pollView.message = out
    ...
    @vote.command(aliases = ["@help"])
    @commands.is_owner()
    async def adminHelp(self, ctx):
        await ctx.send("""
        ```
        insertPoll <string input formatted json>
        clear
        saveReset
        timeConvert
        ```""")
