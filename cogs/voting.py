...
from myutils.rcv.election_coordinator import RCVElectionCoordinator
...
def humantimeTranslator(s):
    ...
    if parts:
        num_seconds = int(parts.group(1)) * mult_items[
            parts.group(2)
        ] + humantimeTranslator(re.sub(r"^(\d+)([^\d]*)", "", s.lower()))
...
class PollButton(discord.ui.Button["Poll"]):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        settingsEmbed = discord.Embed(
            title="Settings & Poll Info", description=content, color=randomHexGen()
        )
        ...
        if self.emoji.name == "settings":
            if str(interaction.user.id) in currPoll:
                currentSettings.settingsEmbed.set_field_at(
                    0,
                    name="Your vote is:",
                    value=str(currPoll.get(str(interaction.user.id))),
                )
            return await interaction.response.send_message(
                embed=settingsEmbed,
                view=Settings(self.currentPoll, currentSettings),
                ephemeral=True,
            )
        ...
        followup = discord.Embed(
            title="You have voted for option: " + str(self.emoji),
...
class SettingsButton(discord.ui.Button["Settings"]):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            isLocked_bool = (
                True if self.pollEmbed.fields[2].value == ":unlock:" else False
            )
        except IndexError:
            closedPoll = True

        if closedPoll and not self.emoji.name == "❔":
            self.settingsEmbed.set_field_at(
                0, name="The poll is now", value="<:cancel:851278899270909993> Closed"
            )
            self.settingsEmbed.set_footer(text="You can dismiss this message now")
            return await interaction.response.edit_message(
                embed=self.settingsEmbed, view=None
            )
        ...
        if self.emoji.name == "🔐":
            ...
            self.settingsEmbed.set_field_at(
                0, name="The poll is now", value=isLocked_str
            )
            ...
            return await interaction.response.edit_message(
                embed=self.settingsEmbed, view=self.view
            )
        ...
        if self.emoji.name == "🕵️‍♂️":
            if self.currentPoll.isAnon:
                self.settingsEmbed.set_field_at(
                    0, name="Results are now", value=":sunny: Shown with names"
                )
                self.pollEmbed.set_author(
                    name="Results are shown with names", icon_url=""
                )
                await self.pollMessage.edit(embed=self.pollEmbed)
                self.currentPoll.isAnon = False
            else:
                self.settingsEmbed.set_field_at(
                    0, name="Results are now", value=":detective: Anonymous"
                )
        ...
        if self.emoji.name == "🖨":
            if currPoll:
                ...
                for key in currPoll:
                    ...
                    results.append(
                        f'[{member.name}](https://www.youtube.com/watch?v=dQw4w9WgXcQ "{member.name}") has voted'
                    )
                embed = discord.Embed(
                    title="Here's a list of people who have voted so far!",
                    description="\n".join(results),
                    color=randomHexGen(),
                )
            else:
                embed = discord.Embed(
                    title="No one has voted yet!", color=randomHexGen()
                )
            return await interaction.response.edit_message(
                embed=embed
            )  # crucial to have a return here
        ...
        if str(self.emoji) == "<:cancel:851278899270909993>":
            ...
            self.settingsEmbed.set_footer(text="You can dismiss this message now")
            try:
                resultsEmbed = await self.clipboardBot.get_command(
                    "createResultsEmbed"
                )(self.currentPoll)
            except Exception:
...
```                                  
        ...
        if str(interaction.user.id) in currPoll:
            ...
            if self.emoji.name == "🔁" and isLocked_bool:
                ...
                writetoFile(currPoll, "storedPolls")
                self.pollEmbed.set_field_at(
                    0, name="Votes Recorded: ", value=len(currPoll)
                )
                self.settingsEmbed.set_field_at(
                    0, name="You haven't voted yet!", value="\u200b"
                )
                await self.pollMessage.edit(embed=self.pollEmbed)
                return await interaction.response.edit_message(
                    embed=self.settingsEmbed, view=self.view
                ...
                self.settingsEmbed.set_field_at(
                    0, name="Poll is :lock:", value="You cannot change your vote"
                )
                return await interaction.response.edit_message(
                    embed=self.settingsEmbed, view=self.view
                )
    ...
    def __init__(self, currentPoll, currentSettings):
        ...
        if currentSettings.isAuthor:  # ? , revote, print, cancel
            settings = [
                ...
                "\U0001f501",
                "\U0001f5a8",
                "<:cancel:851278899270909993>",
            ]
            self.add_item(SelectMenu(currentPoll, currentSettings))
        else:
            settings = ["\U00002754", "\U0001f501", "\U0001f5a8"]  # ?, revote, print
        ...
        for emoji in settings:
            ...
            if (
                currentPoll.isLocked and str(button.emoji) == "\U0001f501"
            ):  # If locked and button refresh
...
class voting(commands.Cog):
    ...
    async def initialize_poll(self, ctx, min_choices):
        ...
        poll_response_view.message = await ctx.send(
            embed=modal_info, view=poll_response_view
        )
        await poll_modal.wait()

        title = poll_modal._title
        emojis, opts, success = await self.bot.get_command("emoji_msg_error_check")(
            ctx, poll_modal.emojis, poll_modal.msg
        )
        ...
        if len(opts) < min_choices:
            raise Exception(
                f"Too few candidates for an election with {min_choices} choices."
            )

        return title, opts, emojis

    async def load_poll(self, ctx, poll):
    ...
    async def vote(self, ctx):
        if ctx.invoked_subcommand is None:
            await ctx.send(
                f"Please specify what you'd like to do. \nEx: `{ctx.prefix}poll create`"
            )
    ...
    def get_fptp_embed(self, ctx, title, opts, emojis, election_time):
        ...
        pollClose = timestamp + timedelta(seconds=election_time)
        embed.add_field(
            name="Votes Recorded:", value=0
        )  # should always be 0 as should ping me if not reset
        embed.add_field(
            name="Date Poll Closes:", value=f"<t:{int(pollClose.timestamp())}:f>"
        )
        ...
        # * Footer
        tips = [
            "Tip #1: Does not work with emojis from outside the current server",
    ...
    async def fptp_make(self, ctx, election_time=86400, poll=None):
        # * Setting up the variables for the embed
        if poll is None:
            title, opts, emojis = await self.initialize_poll(ctx, 1)
    ...
    async def createResultsEmbed(self, currentPoll):
        ...
        if newPoll:
            ...
            the_rest = [
                emoji + " ➙ " + str(freq) + " votes" for emoji, freq in zip(emoji, freq)
            ]
            maxNum = freq[0]
            winnerDict = {
                k: v for k, v in freqCounter.items() if v == maxNum
            }  # Turn it into a dict to cycle through
            ...
            if isAnon:
                ...
                for key, values in newPoll.items():
                    ...
                    results.append(
                        f"[{member.name}](https://www.youtube.com/watch?v=dQw4w9WgXcQ) voted {values}"
                    )
        ...
        if results:
            ...
            if len(winners) == 1:
                ...
                embed.add_field(
                    name=f"The winner (with {maxNum} votes)...",
                    value=final_winner,
                    inline=False,
                )
            else:
                ...
                embed.add_field(
                    name=f"The winners (with {maxNum} votes)...",
                    value="\n".join(winners),
                    inline=False,
                )
                embed.add_field(
                    name="As there was a tie, one has been chosen at random from our winners",
                    value=final_winner,
                    inline=False,
                )
            if not isAnon:
                embed.add_field(
                    name="The totals:", value="\n".join(the_rest), inline=False
                )
            embed.title = f"Our winner is {final_winner}!!"

        embed.add_field(name="Votes Recorded:", value=len(newPoll))
        embed.add_field(
            name="Date Poll Started:", value=f"<t:{int(embed.timestamp.timestamp())}:f>"
        )
        embed.add_field(
            name="Date Poll Closed:",
    ...
    async def print(self, ctx):
        ...
        for key, values in newPoll.items():
            ...
            results.append(
                f'[{member}](https://www.youtube.com/watch?v=dQw4w9WgXcQ "{member.name}") voted {values}'
            )
        privateEmbed = discord.Embed(
    ...
    async def createEmbedManually(self, ctx):
        timestamp = discord.utils.utcnow()
        embed = discord.Embed(
            title="", description="", color=randomHexGen(), timestamp=timestamp
        )
